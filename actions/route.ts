  ...
  try {
    ...
    if (matches.length > 0) {
      context = matches
        .map(
          (m: any) =>
            `${m.metadata?.title || ""}: ${m.metadata?.content || ""}`,
        )
        .join("\n\n");
    } else {
      context =
        "No relevant information was found in the digital twin profile.";
    }
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an AI digital twin. Answer questions as if you are the person, using first person. Be concise and answer only what is asked, without extra information.",
        },
        {
          role: "user",
          content: prompt,
    ...
    return NextResponse.json(
      {
        error:
          err?.message || JSON.stringify(err) || "Error processing request.",
      },
      { status: 500 },
    );
  }
